...
# initialize specified shape variable with normal distribution values
def weight_variable(shape):
	initial = tf.truncated_normal(shape, stddev=0.1)
	return tf.Variable(initial)

# initialize specified shape variables with constant values
...
# ksize size of the window for max computing, strides size of the sliding window
def max_pool_2x2(x):
	return tf.nn.max_pool(x, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

x = tf.placeholder(tf.float32, [None, 784])
# input: [batch, in_height, in_width, in_channels]
x_image = tf.reshape(x, [-1, 28, 28, 1]) # image size 28*28, -1 represent the image number, which is the first dimension of x from input, the last one is color tunnel number
y_ = tf.placeholder(tf.float32, [None, 10])  # 10 digits


### 1. 第一层网络
# 把x_image的厚度由1增加到32，长宽由28*28缩小为14*14
W_conv1 = weight_variable([5, 5, 1, 32])                    # 按照[5,5,输入通道=1,输出通道=32]生成一组随机变量 [filter_height, filter_width, in_channels, out_channels] (5*5 filter with 32)
